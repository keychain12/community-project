                                ...
                                <c:if test="${totalPageCount > 1}">
                                    <ul class="pagination">
                                        <c:if test="${hasPreviousGroup}">
                                            <li class="page-item">
                                                <a class="page-link" href="?page=${startPage - 1}&word=${param.word}">&laquo; 이전</a>
                                            </li>
                                        </c:if>
                                        <c:forEach begin="${startPage}" end="${endPage}" var="i">
                                            <li class="page-item ${i == currentPage ? 'active' : ''}">
                                                <a class="page-link" href="?page=${i}&word=${param.word}">${i}</a>
                                            </li>
                                        </c:forEach>
                                        <c:if test="${hasNextGroup}">
                                            <li class="page-item">
                                                <a class="page-link" href="?page=${endPage + 1}&word=${param.word}">다음 &raquo;</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </c:if>
